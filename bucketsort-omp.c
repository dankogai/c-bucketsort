/*
 * $Id: bucketsort-omp.c,v 0.3 2012/01/13 11:25:59 dankogai Exp dankogai $
 *
...
#pragma omp parallel for
    for (i = 0; i < BUCKETSIZ; i++) {
	if (buckets[i].head) {
	    if (!allthesame(buckets[i], cmp)) {
		buckets[i] =
		    _bucketsort_q(buckets[i], depth + 1, key, idx, cmp);
	    }
	}
    ...
    for (i = 0; i < BUCKETSIZ; i++) {
	if (buckets[i].head)
	    enqueue(&dst, buckets[i].head, buckets[i].tail);
    }
    return dst;
};
